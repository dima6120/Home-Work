...
public class Main {
    public static void main(String[] args) throws LexemeTypeMismatchException, UnexectedLexemException, UnexectedSymbolException, DivByZeroException, TypeMismatchException, UnexpectedTypeException {
        ...
        Node n = pr.parse("let f = fun x -> fun y -> fun z -> fun w -> (x + (y + z)*2+1)*w in let x = 1 in (f (f 1 (1) 3 4) x) (x) ((fun x -> (f) x (1) 1 (x+1)) 1)");
        ...
        Expression e = i.evalExpr((Expression)n);
        System.out.println(e.toString());
        System.out.println("Count: " + Integer.toString(i.getCount()));
    }
}
